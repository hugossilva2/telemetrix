...
## 10. Operação e qualidade (fases 4 e 5)

- **Comando único de verificação:** `bun run check` roda tipos, testes, lint e build.
- **App instalável:** o service worker é gerado em `dist/client/sw.js` (mesma pasta servida
  em produção), garantindo funcionamento offline e avisos na instalação.
- **Prazos nas chamadas externas:** Roads/Geocoding (10 s e 8 s), push (8 s, 5 envios
  simultâneos) e Flespi (15 s por veículo) — um serviço lento não trava o processamento.
- **Leituras paginadas:** `src/lib/supabase/paginate.ts` (`fetchAllRows`) substitui os
  antigos cortes em 500/1.000 registros em relatórios, viagens, corridas, aulas e ranking.
- **Estados de tela:** `src/components/common/LoadFailed.tsx` distingue erro de "sem
  registros", com motivo em português e botão de tentar novamente.
- **Retenção:** rotina diária `purge_old_telemetry()` apaga pontos do rastreador com mais
  de 90 dias e execuções de automação com mais de 180 dias; viagens não são afetadas.
- **Acabamento:** cores dos gráficos usam os tokens OKLCH do tema, o zoom com dois dedos
  está liberado (sem `maximum-scale`) e as telas de erro/404 estão em português.
