...
export async function ingestFlespiMessages(messages: FlespiMessage[]): Promise<IngestSummary> {
      ...
      for (const msg of deviceMessages) {
            ...
            if (places && places.length > 0) {
              ...
              for (const p of places) {
                ...
                if (wasInside === true && inside === false) {
                  ...
                  await sendTrackerEventPush(vehicle.user_id as string, "geofence_exit", {
                    placeName: (p.name as string) ?? null,
                    vehicleId: vehicle.id as string,
                  });
                  ...
                  await sendTrackerEventPush(vehicle.user_id as string, "geofence_enter", {
                    placeName: (p.name as string) ?? null,
                    vehicleId: vehicle.id as string,
                  });
                  await fireAutomationsForPlace(supabaseAdmin as never, {
        ...
        if (shouldClose && state) {
          ...
          const durationS = Math.max(
            0,
            Math.round(
              (new Date(nowIso).getTime() - new Date(state.start_time as string).getTime()) / 1000,
            ),
          );
        ...
        if (ign === true && state?.start_time != null) {
          ...
          await supabaseAdmin
            ...
            .update({
              ...
              mileage_at_start: state.mileage_at_start ?? mileage ?? null,
              accum_distance_km: Number(state.accum_distance_km ?? 0) + increment,
              updated_at: nowIso,
              last_message_at: nowIso,
              ...(pingWritten ? { last_ping_at: nowIso } : {}),
            })
