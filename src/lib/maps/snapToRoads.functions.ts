...
export type SnappedPoint = {
  lat: number;
  lng: number;
  /** Índice do ponto original que originou este ponto (undefined = interpolado). */
...
async function snapBatch(points: SnapInput[], offset: number): Promise<SnappedPoint[]> {
  ...
  return (json.snappedPoints ?? [])
    ...
    .map((p) => ({
      ...
      originalIndex:
        typeof p.originalIndex === "number" ? p.originalIndex + offset : null,
    }));
}

/**
 * Alinha a lista de coordenadas GPS à geometria real das ruas (Google Roads API).
 * Processa em lotes de 100 pontos. Nunca lança para o app: em caso de falha
 * retorna `snapped: false` para o chamador cair no traçado bruto.
 */
export const snapToRoads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<SnapResult> => {
    ...
    // Limite defensivo: ~50 lotes (5.000 pontos).
    const points = raw.length > 5000 ? raw.slice(-5000) : raw;
